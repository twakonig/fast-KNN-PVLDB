...
def createSpeedupPlot(N, s1, s2, s3, s4, flags, name):
    ...
    plt.plot(N, s2, marker='o', ms=7, c='darkgreen', linewidth=2.2, linestyle='solid', label='l2-norm + heap (iterative)')
    plt.plot(N, s3, marker='o', ms=7, c='darkblue', linewidth=2.2, linestyle='solid', label='l2-norm + heap (iterative) + col_mean')
    plt.plot(N, s4, marker='o', ms=7, c='brown', linewidth=2.2, linestyle='solid', label='l2-norm + heap (iterative) + col_mean + knn_utility (inlining)')
    ...
    plt.savefig('./plots/speedup/a2_scalar_speedup_' + name + '.pdf')
...
opt1_v = pd.read_csv('./data/alg2_opts/scalar/scalar_alg2opt1_O3-ffm_cycles.csv').values
opt2_v = pd.read_csv('./data/alg2_opts/scalar/scalar_alg2opt2_O3-ffm_cycles.csv').values
opt3_v = pd.read_csv('./data/alg2_opts/scalar/scalar_alg2opt3_O3-ffm_cycles.csv').values
opt4_v = pd.read_csv('./data/alg2_opts/scalar/scalar_alg2opt4_O3-ffm_cycles.csv').values


# number of data points with 30 measurement runs (afterwards only 3 runs per input dimension)
# after that takes median of 3 runs -> check .csv file!! but code will complain because of dimensions if this is wrong
base_runs_30 = 4
opt_runs_30 = 4
N = getN(base_O3v)
...
createSpeedupPlot(N, speedup1v, speedup2v, speedup3v, speedup4v, flags_scalar_O3_ffm, 'O3_ffm')
