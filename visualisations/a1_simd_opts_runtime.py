...
def createRuntimePlot(N, base, baseO3v, y_1, y_2, y_3, y_4, y_5, flags, name):
    ...
    plt.plot(N, baseO3v, marker='o', ms=7, c='olive', linewidth=2.2, linestyle='dashed', label=base_v)
    plt.plot(N, y_1, marker='o', ms=7, c='orange', linewidth=2.2, label='l2-norm (SIMD)')
    plt.plot(N, y_2, marker='o', ms=7, c='darkblue', linewidth=2.2, linestyle='solid', label='l2-norm (SIMD) + quadsort')
    #plt.plot(N, y_3, marker='o', ms=7, c='darkgreen', linewidth=2.2, linestyle='dashdot', label='opt3')
    plt.plot(N, y_4, marker='o', ms=7, c='darkgreen', linewidth=2.2, linestyle='solid', label='l2-norm (SIMD) + quadsort + get_true_knn (unrolling & referencing) + compute_shapley')
    plt.plot(N, y_5, marker='o', ms=7, c='purple', linewidth=2.2, linestyle='solid', label='quadsort + get_true_knn (SIMD blocking) + compute_shapley')
    ...
    #plt.text(10,10, r'$\sum_{i=0}^\infty x_i$', fontsize=20)
    plt.tight_layout()
    ...
    ax.set_ylim([10**4.5, 10**(13.5)])

    #plt.savefig('./a1_scalar_opts_' + name + '.pdf')
    plt.savefig('./plots/runtime/a1_vectorized_opts_' + name + '.pdf')
...
createRuntimePlot(N, t_base, t_baseO3v, t_o1v, t_o2v, t_o3v, t_o4v, t_o5v, flags_vectorized_O3, 'O3')
