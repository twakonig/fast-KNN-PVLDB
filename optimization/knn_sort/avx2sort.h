//
// Created by lucasck on 22/05/22.
//
...
#define LOAD_VECTOR(arr) _mm256_loadu_ps(reinterpret_cast<__m256 *>(arr))
#define STORE_VECTOR(arr, vec)                                                   \
  _mm256_storeu_ps(reinterpret_cast<__m256 *>(arr), vec)
...
#define COEX(a, b){                                                   \
    auto vec_tmp = a;                                                          \
    a = _mm256_min_ps(a, b);                                                \
    b = _mm256_max_ps(vec_tmp, b);}

/* shuffle 2 vectors, instruction for int is missing,
 * therefore shuffle with float */
#define SHUFFLE_2_VECS(a, b, mask)(_mm256_shuffle_ps(a, b, mask));

/* optimized sorting network for two vectors, that is 16 ints */
            inline void sort_16(__m256 &v1, __m256 &v2) {
                COEX(v1, v2);                                  /* step 1 */

                v2 = _mm256_shuffle_ps(v2, v2, _MM_SHUFFLE(2, 3, 0, 1)); /* step 2 */
                COEX(v1, v2);
                ...
                v2 = _mm256_shuffle_ps(v2, v2, _MM_SHUFFLE(0, 1, 2, 3)); /* step  4 */
                COEX(v1, v2);
                ...
                v2 = _mm256_permutevar8x32_ps(v2, _mm256_setr_ps(7,6,5,4,3,2,1,0));
                COEX(v1, v2);                                           /* step  7 */
                ...
                v1 = _mm256_permutevar8x32_ps(v1, _mm256_setr_ps(0,4,1,5,6,2,7,3));
                v2 = _mm256_permutevar8x32_ps(v2, _mm256_setr_ps(0,4,1,5,6,2,7,3));
                ...
                /* restore order */
                auto b2 = _mm256_shuffle_ps(v2,v2, 0b10110001);
                auto b1 = _mm256_shuffle_ps(v1,v1, 0b10110001);
                v1 = _mm256_blend_ps(v1, b2, 0b10101010);
                v2 = _mm256_blend_ps(b1, v2, 0b10101010);
            }
...
#define COEX_PERMUTE(vec, a, b, c, d, e, f, g, h, MASK){               \
    __m256 permute_mask = _mm256_setr_ps(a, b, c, d, e, f, g, h);  \
    __m256 permuted = _mm256_permutevar8x32_ps(vec, permute_mask); \
    __m256 min = _mm256_min_ps(permuted, vec);                     \
    __m256 max = _mm256_max_ps(permuted, vec);                     \
    constexpr int blend_mask = MASK(a, b, c, d, e, f, g, h);           \
    vec = _mm256_blend_ps(min, max, blend_mask);}

#define COEX_SHUFFLE(vec, a, b, c, d, e, f, g, h, MASK){               \
    constexpr int shuffle_mask = _MM_SHUFFLE(d, c, b, a);              \
    __m256 shuffled = _mm256_shuffle_ps(vec, vec, shuffle_mask);        \
    __m256 min = _mm256_min_ps(shuffled, vec);                     \
    __m256 max = _mm256_max_ps(shuffled, vec);                     \
    constexpr int blend_mask = MASK(a, b, c, d, e, f, g, h);           \
    vec = _mm256_blend_ps(min, max, blend_mask);}

#define REVERSE_VEC(vec){                                              \
    vec = _mm256_permutevar8x32_ps(                                 \
        vec, _mm256_setr_ps(7, 6, 5, 4, 3, 2, 1, 0));}
 ...
 * s = 2 means that two vectors are already sorted */
            inline void bitonic_merge_16(__m256 *vecs, const int N, const int s = 2) {
                for (int t = s * 2; t < 2 * N; t *= 2) {
                    ...
                    for (int i = 0; i < N; i += 2) {
                        ...
                        auto tmp = vecs[i];
                        vecs[i] = _mm256_unpacklo_ps(vecs[i], vecs[i + 1]);
                        vecs[i + 1] = _mm256_unpackhi_ps(tmp, vecs[i + 1]);
            ...
            inline void bitonic_merge_128(__m256 *vecs, const int N, const int s = 16) {
                ...
                for (int t = s * 2; t < 2 * N; t *= 2) {
                    ...
                    for (int i = 0; i < N; i += 2) {
                        ...
                        COEX_PERMUTE(vecs[i + 1], 4, 5, 6, 7, 0, 1, 2, 3, ASC);
                        auto tmp = vecs[i];
                        vecs[i] = _mm256_unpacklo_ps32(vecs[i], vecs[i + 1]);
                        vecs[i + 1] = _mm256_unpackhi_ps(tmp, vecs[i + 1]);
                        COEX(vecs[i], vecs[i + 1]);
                        tmp = vecs[i];
                        vecs[i] = _mm256_unpacklo_ps32(vecs[i], vecs[i + 1]);
...
/* sort 8 columns each containing 16 int, with 60 modules */
            inline void sort_16_int_vertical(__m256* vecs){
 ...
 * without transposition */
            void inline merge_8_columns_with_16_elements(__m256* vecs){
                vecs[8] = _mm256_shuffle_ps(vecs[8],vecs[8], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[7], vecs[8]);
                vecs[9] = _mm256_shuffle_ps(vecs[9],vecs[9], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[6], vecs[9]);
                vecs[10] = _mm256_shuffle_ps(vecs[10],vecs[10], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[5], vecs[10]);
                vecs[11] = _mm256_shuffle_ps(vecs[11],vecs[11], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[4], vecs[11]);
                vecs[12] = _mm256_shuffle_ps(vecs[12],vecs[12], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[3], vecs[12]);
                vecs[13] = _mm256_shuffle_ps(vecs[13],vecs[13], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[2], vecs[13]);
                vecs[14] = _mm256_shuffle_ps(vecs[14],vecs[14], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[1], vecs[14]);
                vecs[15] = _mm256_shuffle_ps(vecs[15],vecs[15], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[0], vecs[15]);
                vecs[4] = _mm256_shuffle_ps(vecs[4],vecs[4], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[3], vecs[4]);
                vecs[5] = _mm256_shuffle_ps(vecs[5],vecs[5], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[2], vecs[5]);
                vecs[6] = _mm256_shuffle_ps(vecs[6],vecs[6], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[1], vecs[6]);
                vecs[7] = _mm256_shuffle_ps(vecs[7],vecs[7], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[0], vecs[7]);
                vecs[12] = _mm256_shuffle_ps(vecs[12],vecs[12], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[11], vecs[12]);
                vecs[13] = _mm256_shuffle_ps(vecs[13],vecs[13], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[10], vecs[13]);
                vecs[14] = _mm256_shuffle_ps(vecs[14],vecs[14], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[9], vecs[14]);
                vecs[15] = _mm256_shuffle_ps(vecs[15],vecs[15], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[8], vecs[15]);
                vecs[2] = _mm256_shuffle_ps(vecs[2],vecs[2], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[1], vecs[2]);
                vecs[3] = _mm256_shuffle_ps(vecs[3],vecs[3], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[0], vecs[3]);
                vecs[6] = _mm256_shuffle_ps(vecs[6],vecs[6], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[5], vecs[6]);
                vecs[7] = _mm256_shuffle_ps(vecs[7],vecs[7], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[4], vecs[7]);
                vecs[10] = _mm256_shuffle_ps(vecs[10],vecs[10], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[9], vecs[10]);
                vecs[11] = _mm256_shuffle_ps(vecs[11],vecs[11], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[8], vecs[11]);
                vecs[14] = _mm256_shuffle_ps(vecs[14],vecs[14], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[13], vecs[14]);
                vecs[15] = _mm256_shuffle_ps(vecs[15],vecs[15], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[12], vecs[15]);
                vecs[1] = _mm256_shuffle_ps(vecs[1],vecs[1], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[0], vecs[1]);
                vecs[3] = _mm256_shuffle_ps(vecs[3],vecs[3], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[2], vecs[3]);
                vecs[5] = _mm256_shuffle_ps(vecs[5],vecs[5], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[4], vecs[5]);
                vecs[7] = _mm256_shuffle_ps(vecs[7],vecs[7], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[6], vecs[7]);
                vecs[9] = _mm256_shuffle_ps(vecs[9],vecs[9], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[8], vecs[9]);
                vecs[11] = _mm256_shuffle_ps(vecs[11],vecs[11], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[10], vecs[11]);
                vecs[13] = _mm256_shuffle_ps(vecs[13],vecs[13], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[12], vecs[13]);
                vecs[15] = _mm256_shuffle_ps(vecs[15],vecs[15], _MM_SHUFFLE(2,3,0,1)); COEX(vecs[14], vecs[15]);
                ...
                vecs[8] = _mm256_shuffle_ps(vecs[8],vecs[8], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[7], vecs[8]);
                vecs[9] = _mm256_shuffle_ps(vecs[9],vecs[9], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[6], vecs[9]);
                vecs[10] = _mm256_shuffle_ps(vecs[10],vecs[10], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[5], vecs[10]);
                vecs[11] = _mm256_shuffle_ps(vecs[11],vecs[11], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[4], vecs[11]);
                vecs[12] = _mm256_shuffle_ps(vecs[12],vecs[12], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[3], vecs[12]);
                vecs[13] = _mm256_shuffle_ps(vecs[13],vecs[13], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[2], vecs[13]);
                vecs[14] = _mm256_shuffle_ps(vecs[14],vecs[14], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[1], vecs[14]);
                vecs[15] = _mm256_shuffle_ps(vecs[15],vecs[15], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[0], vecs[15]);
                vecs[4] = _mm256_shuffle_ps(vecs[4],vecs[4], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[3], vecs[4]);
                vecs[5] = _mm256_shuffle_ps(vecs[5],vecs[5], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[2], vecs[5]);
                vecs[6] = _mm256_shuffle_ps(vecs[6],vecs[6], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[1], vecs[6]);
                vecs[7] = _mm256_shuffle_ps(vecs[7],vecs[7], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[0], vecs[7]);
                vecs[12] = _mm256_shuffle_ps(vecs[12],vecs[12], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[11], vecs[12]);
                vecs[13] = _mm256_shuffle_ps(vecs[13],vecs[13], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[10], vecs[13]);
                vecs[14] = _mm256_shuffle_ps(vecs[14],vecs[14], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[9], vecs[14]);
                vecs[15] = _mm256_shuffle_ps(vecs[15],vecs[15], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[8], vecs[15]);
                vecs[2] = _mm256_shuffle_ps(vecs[2],vecs[2], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[1], vecs[2]);
                vecs[3] = _mm256_shuffle_ps(vecs[3],vecs[3], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[0], vecs[3]);
                vecs[6] = _mm256_shuffle_ps(vecs[6],vecs[6], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[5], vecs[6]);
                vecs[7] = _mm256_shuffle_ps(vecs[7],vecs[7], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[4], vecs[7]);
                vecs[10] = _mm256_shuffle_ps(vecs[10],vecs[10], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[9], vecs[10]);
                vecs[11] = _mm256_shuffle_ps(vecs[11],vecs[11], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[8], vecs[11]);
                vecs[14] = _mm256_shuffle_ps(vecs[14],vecs[14], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[13], vecs[14]);
                vecs[15] = _mm256_shuffle_ps(vecs[15],vecs[15], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[12], vecs[15]);
                vecs[1] = _mm256_shuffle_ps(vecs[1],vecs[1], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[0], vecs[1]);
                vecs[3] = _mm256_shuffle_ps(vecs[3],vecs[3], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[2], vecs[3]);
                vecs[5] = _mm256_shuffle_ps(vecs[5],vecs[5], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[4], vecs[5]);
                vecs[7] = _mm256_shuffle_ps(vecs[7],vecs[7], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[6], vecs[7]);
                vecs[9] = _mm256_shuffle_ps(vecs[9],vecs[9], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[8], vecs[9]);
                vecs[11] = _mm256_shuffle_ps(vecs[11],vecs[11], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[10], vecs[11]);
                vecs[13] = _mm256_shuffle_ps(vecs[13],vecs[13], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[12], vecs[13]);
                vecs[15] = _mm256_shuffle_ps(vecs[15],vecs[15], _MM_SHUFFLE(0,1,2,3)); COEX(vecs[14], vecs[15]);
            ...
            inline void sort_int_sorting_network(float *arr, float *buff, int n) {
                if(n < 2) return;
                __m256 *buffer = reinterpret_cast<__m256 *>(buff);

                const auto remainder = int(n % 8 ? n % 8 : 8);
                const int idx_max_pad = n - remainder;
                const auto mask = _mm256_add_ps(_mm256_set1_ps(-remainder), _mm256_setr_ps(0, 1, 2, 3, 4, 5, 6, 7));
                auto max_pad_vec = _mm256_blendv_ps(_mm256_set1_ps(INT32_MAX),_mm256_maskload_ps(arr + idx_max_pad, mask), mask);

                for (int i = 0; i < idx_max_pad / 8; ++i) {
                    buffer[i] = LOAD_VECTOR(arr + i * 8);
                }
                buffer[idx_max_pad / 8] = max_pad_vec;
                buffer[idx_max_pad / 8 + 1] = _mm256_set1_ps(INT32_MAX);
                ...
                _mm256_maskstore_ps(arr + idx_max_pad, mask, buffer[idx_max_pad / 8]);
            }
...
/* auto generated permutations masks for quicksort(8 KB..) */
            const __m256 permutation_masks[256] = {_mm256_setr_ps(0, 1, 2, 3, 4, 5, 6, 7),
                                                    _mm256_setr_ps(1, 2, 3, 4, 5, 6, 7, 0),
                                                    _mm256_setr_ps(0, 2, 3, 4, 5, 6, 7, 1),
                                                    _mm256_setr_ps(2, 3, 4, 5, 6, 7, 0, 1),
                                                    _mm256_setr_ps(0, 1, 3, 4, 5, 6, 7, 2),
                                                    _mm256_setr_ps(1, 3, 4, 5, 6, 7, 0, 2),
                                                    _mm256_setr_ps(0, 3, 4, 5, 6, 7, 1, 2),
                                                    _mm256_setr_ps(3, 4, 5, 6, 7, 0, 1, 2),
                                                    _mm256_setr_ps(0, 1, 2, 4, 5, 6, 7, 3),
                                                    _mm256_setr_ps(1, 2, 4, 5, 6, 7, 0, 3),
                                                    _mm256_setr_ps(0, 2, 4, 5, 6, 7, 1, 3),
                                                    _mm256_setr_ps(2, 4, 5, 6, 7, 0, 1, 3),
                                                    _mm256_setr_ps(0, 1, 4, 5, 6, 7, 2, 3),
                                                    _mm256_setr_ps(1, 4, 5, 6, 7, 0, 2, 3),
                                                    _mm256_setr_ps(0, 4, 5, 6, 7, 1, 2, 3),
                                                    _mm256_setr_ps(4, 5, 6, 7, 0, 1, 2, 3),
                                                    _mm256_setr_ps(0, 1, 2, 3, 5, 6, 7, 4),
                                                    _mm256_setr_ps(1, 2, 3, 5, 6, 7, 0, 4),
                                                    _mm256_setr_ps(0, 2, 3, 5, 6, 7, 1, 4),
                                                    _mm256_setr_ps(2, 3, 5, 6, 7, 0, 1, 4),
                                                    _mm256_setr_ps(0, 1, 3, 5, 6, 7, 2, 4),
                                                    _mm256_setr_ps(1, 3, 5, 6, 7, 0, 2, 4),
                                                    _mm256_setr_ps(0, 3, 5, 6, 7, 1, 2, 4),
                                                    _mm256_setr_ps(3, 5, 6, 7, 0, 1, 2, 4),
                                                    _mm256_setr_ps(0, 1, 2, 5, 6, 7, 3, 4),
                                                    _mm256_setr_ps(1, 2, 5, 6, 7, 0, 3, 4),
                                                    _mm256_setr_ps(0, 2, 5, 6, 7, 1, 3, 4),
                                                    _mm256_setr_ps(2, 5, 6, 7, 0, 1, 3, 4),
                                                    _mm256_setr_ps(0, 1, 5, 6, 7, 2, 3, 4),
                                                    _mm256_setr_ps(1, 5, 6, 7, 0, 2, 3, 4),
                                                    _mm256_setr_ps(0, 5, 6, 7, 1, 2, 3, 4),
                                                    _mm256_setr_ps(5, 6, 7, 0, 1, 2, 3, 4),
                                                    _mm256_setr_ps(0, 1, 2, 3, 4, 6, 7, 5),
                                                    _mm256_setr_ps(1, 2, 3, 4, 6, 7, 0, 5),
                                                    _mm256_setr_ps(0, 2, 3, 4, 6, 7, 1, 5),
                                                    _mm256_setr_ps(2, 3, 4, 6, 7, 0, 1, 5),
                                                    _mm256_setr_ps(0, 1, 3, 4, 6, 7, 2, 5),
                                                    _mm256_setr_ps(1, 3, 4, 6, 7, 0, 2, 5),
                                                    _mm256_setr_ps(0, 3, 4, 6, 7, 1, 2, 5),
                                                    _mm256_setr_ps(3, 4, 6, 7, 0, 1, 2, 5),
                                                    _mm256_setr_ps(0, 1, 2, 4, 6, 7, 3, 5),
                                                    _mm256_setr_ps(1, 2, 4, 6, 7, 0, 3, 5),
                                                    _mm256_setr_ps(0, 2, 4, 6, 7, 1, 3, 5),
                                                    _mm256_setr_ps(2, 4, 6, 7, 0, 1, 3, 5),
                                                    _mm256_setr_ps(0, 1, 4, 6, 7, 2, 3, 5),
                                                    _mm256_setr_ps(1, 4, 6, 7, 0, 2, 3, 5),
                                                    _mm256_setr_ps(0, 4, 6, 7, 1, 2, 3, 5),
                                                    _mm256_setr_ps(4, 6, 7, 0, 1, 2, 3, 5),
                                                    _mm256_setr_ps(0, 1, 2, 3, 6, 7, 4, 5),
                                                    _mm256_setr_ps(1, 2, 3, 6, 7, 0, 4, 5),
                                                    _mm256_setr_ps(0, 2, 3, 6, 7, 1, 4, 5),
                                                    _mm256_setr_ps(2, 3, 6, 7, 0, 1, 4, 5),
                                                    _mm256_setr_ps(0, 1, 3, 6, 7, 2, 4, 5),
                                                    _mm256_setr_ps(1, 3, 6, 7, 0, 2, 4, 5),
                                                    _mm256_setr_ps(0, 3, 6, 7, 1, 2, 4, 5),
                                                    _mm256_setr_ps(3, 6, 7, 0, 1, 2, 4, 5),
                                                    _mm256_setr_ps(0, 1, 2, 6, 7, 3, 4, 5),
                                                    _mm256_setr_ps(1, 2, 6, 7, 0, 3, 4, 5),
                                                    _mm256_setr_ps(0, 2, 6, 7, 1, 3, 4, 5),
                                                    _mm256_setr_ps(2, 6, 7, 0, 1, 3, 4, 5),
                                                    _mm256_setr_ps(0, 1, 6, 7, 2, 3, 4, 5),
                                                    _mm256_setr_ps(1, 6, 7, 0, 2, 3, 4, 5),
                                                    _mm256_setr_ps(0, 6, 7, 1, 2, 3, 4, 5),
                                                    _mm256_setr_ps(6, 7, 0, 1, 2, 3, 4, 5),
                                                    _mm256_setr_ps(0, 1, 2, 3, 4, 5, 7, 6),
                                                    _mm256_setr_ps(1, 2, 3, 4, 5, 7, 0, 6),
                                                    _mm256_setr_ps(0, 2, 3, 4, 5, 7, 1, 6),
                                                    _mm256_setr_ps(2, 3, 4, 5, 7, 0, 1, 6),
                                                    _mm256_setr_ps(0, 1, 3, 4, 5, 7, 2, 6),
                                                    _mm256_setr_ps(1, 3, 4, 5, 7, 0, 2, 6),
                                                    _mm256_setr_ps(0, 3, 4, 5, 7, 1, 2, 6),
                                                    _mm256_setr_ps(3, 4, 5, 7, 0, 1, 2, 6),
                                                    _mm256_setr_ps(0, 1, 2, 4, 5, 7, 3, 6),
                                                    _mm256_setr_ps(1, 2, 4, 5, 7, 0, 3, 6),
                                                    _mm256_setr_ps(0, 2, 4, 5, 7, 1, 3, 6),
                                                    _mm256_setr_ps(2, 4, 5, 7, 0, 1, 3, 6),
                                                    _mm256_setr_ps(0, 1, 4, 5, 7, 2, 3, 6),
                                                    _mm256_setr_ps(1, 4, 5, 7, 0, 2, 3, 6),
                                                    _mm256_setr_ps(0, 4, 5, 7, 1, 2, 3, 6),
                                                    _mm256_setr_ps(4, 5, 7, 0, 1, 2, 3, 6),
                                                    _mm256_setr_ps(0, 1, 2, 3, 5, 7, 4, 6),
                                                    _mm256_setr_ps(1, 2, 3, 5, 7, 0, 4, 6),
                                                    _mm256_setr_ps(0, 2, 3, 5, 7, 1, 4, 6),
                                                    _mm256_setr_ps(2, 3, 5, 7, 0, 1, 4, 6),
                                                    _mm256_setr_ps(0, 1, 3, 5, 7, 2, 4, 6),
                                                    _mm256_setr_ps(1, 3, 5, 7, 0, 2, 4, 6),
                                                    _mm256_setr_ps(0, 3, 5, 7, 1, 2, 4, 6),
                                                    _mm256_setr_ps(3, 5, 7, 0, 1, 2, 4, 6),
                                                    _mm256_setr_ps(0, 1, 2, 5, 7, 3, 4, 6),
                                                    _mm256_setr_ps(1, 2, 5, 7, 0, 3, 4, 6),
                                                    _mm256_setr_ps(0, 2, 5, 7, 1, 3, 4, 6),
                                                    _mm256_setr_ps(2, 5, 7, 0, 1, 3, 4, 6),
                                                    _mm256_setr_ps(0, 1, 5, 7, 2, 3, 4, 6),
                                                    _mm256_setr_ps(1, 5, 7, 0, 2, 3, 4, 6),
                                                    _mm256_setr_ps(0, 5, 7, 1, 2, 3, 4, 6),
                                                    _mm256_setr_ps(5, 7, 0, 1, 2, 3, 4, 6),
                                                    _mm256_setr_ps(0, 1, 2, 3, 4, 7, 5, 6),
                                                    _mm256_setr_ps(1, 2, 3, 4, 7, 0, 5, 6),
                                                    _mm256_setr_ps(0, 2, 3, 4, 7, 1, 5, 6),
                                                    _mm256_setr_ps(2, 3, 4, 7, 0, 1, 5, 6),
                                                    _mm256_setr_ps(0, 1, 3, 4, 7, 2, 5, 6),
                                                    _mm256_setr_ps(1, 3, 4, 7, 0, 2, 5, 6),
                                                    _mm256_setr_ps(0, 3, 4, 7, 1, 2, 5, 6),
                                                    _mm256_setr_ps(3, 4, 7, 0, 1, 2, 5, 6),
                                                    _mm256_setr_ps(0, 1, 2, 4, 7, 3, 5, 6),
                                                    _mm256_setr_ps(1, 2, 4, 7, 0, 3, 5, 6),
                                                    _mm256_setr_ps(0, 2, 4, 7, 1, 3, 5, 6),
                                                    _mm256_setr_ps(2, 4, 7, 0, 1, 3, 5, 6),
                                                    _mm256_setr_ps(0, 1, 4, 7, 2, 3, 5, 6),
                                                    _mm256_setr_ps(1, 4, 7, 0, 2, 3, 5, 6),
                                                    _mm256_setr_ps(0, 4, 7, 1, 2, 3, 5, 6),
                                                    _mm256_setr_ps(4, 7, 0, 1, 2, 3, 5, 6),
                                                    _mm256_setr_ps(0, 1, 2, 3, 7, 4, 5, 6),
                                                    _mm256_setr_ps(1, 2, 3, 7, 0, 4, 5, 6),
                                                    _mm256_setr_ps(0, 2, 3, 7, 1, 4, 5, 6),
                                                    _mm256_setr_ps(2, 3, 7, 0, 1, 4, 5, 6),
                                                    _mm256_setr_ps(0, 1, 3, 7, 2, 4, 5, 6),
                                                    _mm256_setr_ps(1, 3, 7, 0, 2, 4, 5, 6),
                                                    _mm256_setr_ps(0, 3, 7, 1, 2, 4, 5, 6),
                                                    _mm256_setr_ps(3, 7, 0, 1, 2, 4, 5, 6),
                                                    _mm256_setr_ps(0, 1, 2, 7, 3, 4, 5, 6),
                                                    _mm256_setr_ps(1, 2, 7, 0, 3, 4, 5, 6),
                                                    _mm256_setr_ps(0, 2, 7, 1, 3, 4, 5, 6),
                                                    _mm256_setr_ps(2, 7, 0, 1, 3, 4, 5, 6),
                                                    _mm256_setr_ps(0, 1, 7, 2, 3, 4, 5, 6),
                                                    _mm256_setr_ps(1, 7, 0, 2, 3, 4, 5, 6),
                                                    _mm256_setr_ps(0, 7, 1, 2, 3, 4, 5, 6),
                                                    _mm256_setr_ps(7, 0, 1, 2, 3, 4, 5, 6),
                                                    _mm256_setr_ps(0, 1, 2, 3, 4, 5, 6, 7),
                                                    _mm256_setr_ps(1, 2, 3, 4, 5, 6, 0, 7),
                                                    _mm256_setr_ps(0, 2, 3, 4, 5, 6, 1, 7),
                                                    _mm256_setr_ps(2, 3, 4, 5, 6, 0, 1, 7),
                                                    _mm256_setr_ps(0, 1, 3, 4, 5, 6, 2, 7),
                                                    _mm256_setr_ps(1, 3, 4, 5, 6, 0, 2, 7),
                                                    _mm256_setr_ps(0, 3, 4, 5, 6, 1, 2, 7),
                                                    _mm256_setr_ps(3, 4, 5, 6, 0, 1, 2, 7),
                                                    _mm256_setr_ps(0, 1, 2, 4, 5, 6, 3, 7),
                                                    _mm256_setr_ps(1, 2, 4, 5, 6, 0, 3, 7),
                                                    _mm256_setr_ps(0, 2, 4, 5, 6, 1, 3, 7),
                                                    _mm256_setr_ps(2, 4, 5, 6, 0, 1, 3, 7),
                                                    _mm256_setr_ps(0, 1, 4, 5, 6, 2, 3, 7),
                                                    _mm256_setr_ps(1, 4, 5, 6, 0, 2, 3, 7),
                                                    _mm256_setr_ps(0, 4, 5, 6, 1, 2, 3, 7),
                                                    _mm256_setr_ps(4, 5, 6, 0, 1, 2, 3, 7),
                                                    _mm256_setr_ps(0, 1, 2, 3, 5, 6, 4, 7),
                                                    _mm256_setr_ps(1, 2, 3, 5, 6, 0, 4, 7),
                                                    _mm256_setr_ps(0, 2, 3, 5, 6, 1, 4, 7),
                                                    _mm256_setr_ps(2, 3, 5, 6, 0, 1, 4, 7),
                                                    _mm256_setr_ps(0, 1, 3, 5, 6, 2, 4, 7),
                                                    _mm256_setr_ps(1, 3, 5, 6, 0, 2, 4, 7),
                                                    _mm256_setr_ps(0, 3, 5, 6, 1, 2, 4, 7),
                                                    _mm256_setr_ps(3, 5, 6, 0, 1, 2, 4, 7),
                                                    _mm256_setr_ps(0, 1, 2, 5, 6, 3, 4, 7),
                                                    _mm256_setr_ps(1, 2, 5, 6, 0, 3, 4, 7),
                                                    _mm256_setr_ps(0, 2, 5, 6, 1, 3, 4, 7),
                                                    _mm256_setr_ps(2, 5, 6, 0, 1, 3, 4, 7),
                                                    _mm256_setr_ps(0, 1, 5, 6, 2, 3, 4, 7),
                                                    _mm256_setr_ps(1, 5, 6, 0, 2, 3, 4, 7),
                                                    _mm256_setr_ps(0, 5, 6, 1, 2, 3, 4, 7),
                                                    _mm256_setr_ps(5, 6, 0, 1, 2, 3, 4, 7),
                                                    _mm256_setr_ps(0, 1, 2, 3, 4, 6, 5, 7),
                                                    _mm256_setr_ps(1, 2, 3, 4, 6, 0, 5, 7),
                                                    _mm256_setr_ps(0, 2, 3, 4, 6, 1, 5, 7),
                                                    _mm256_setr_ps(2, 3, 4, 6, 0, 1, 5, 7),
                                                    _mm256_setr_ps(0, 1, 3, 4, 6, 2, 5, 7),
                                                    _mm256_setr_ps(1, 3, 4, 6, 0, 2, 5, 7),
                                                    _mm256_setr_ps(0, 3, 4, 6, 1, 2, 5, 7),
                                                    _mm256_setr_ps(3, 4, 6, 0, 1, 2, 5, 7),
                                                    _mm256_setr_ps(0, 1, 2, 4, 6, 3, 5, 7),
                                                    _mm256_setr_ps(1, 2, 4, 6, 0, 3, 5, 7),
                                                    _mm256_setr_ps(0, 2, 4, 6, 1, 3, 5, 7),
                                                    _mm256_setr_ps(2, 4, 6, 0, 1, 3, 5, 7),
                                                    _mm256_setr_ps(0, 1, 4, 6, 2, 3, 5, 7),
                                                    _mm256_setr_ps(1, 4, 6, 0, 2, 3, 5, 7),
                                                    _mm256_setr_ps(0, 4, 6, 1, 2, 3, 5, 7),
                                                    _mm256_setr_ps(4, 6, 0, 1, 2, 3, 5, 7),
                                                    _mm256_setr_ps(0, 1, 2, 3, 6, 4, 5, 7),
                                                    _mm256_setr_ps(1, 2, 3, 6, 0, 4, 5, 7),
                                                    _mm256_setr_ps(0, 2, 3, 6, 1, 4, 5, 7),
                                                    _mm256_setr_ps(2, 3, 6, 0, 1, 4, 5, 7),
                                                    _mm256_setr_ps(0, 1, 3, 6, 2, 4, 5, 7),
                                                    _mm256_setr_ps(1, 3, 6, 0, 2, 4, 5, 7),
                                                    _mm256_setr_ps(0, 3, 6, 1, 2, 4, 5, 7),
                                                    _mm256_setr_ps(3, 6, 0, 1, 2, 4, 5, 7),
                                                    _mm256_setr_ps(0, 1, 2, 6, 3, 4, 5, 7),
                                                    _mm256_setr_ps(1, 2, 6, 0, 3, 4, 5, 7),
                                                    _mm256_setr_ps(0, 2, 6, 1, 3, 4, 5, 7),
                                                    _mm256_setr_ps(2, 6, 0, 1, 3, 4, 5, 7),
                                                    _mm256_setr_ps(0, 1, 6, 2, 3, 4, 5, 7),
                                                    _mm256_setr_ps(1, 6, 0, 2, 3, 4, 5, 7),
                                                    _mm256_setr_ps(0, 6, 1, 2, 3, 4, 5, 7),
                                                    _mm256_setr_ps(6, 0, 1, 2, 3, 4, 5, 7),
                                                    _mm256_setr_ps(0, 1, 2, 3, 4, 5, 6, 7),
                                                    _mm256_setr_ps(1, 2, 3, 4, 5, 0, 6, 7),
                                                    _mm256_setr_ps(0, 2, 3, 4, 5, 1, 6, 7),
                                                    _mm256_setr_ps(2, 3, 4, 5, 0, 1, 6, 7),
                                                    _mm256_setr_ps(0, 1, 3, 4, 5, 2, 6, 7),
                                                    _mm256_setr_ps(1, 3, 4, 5, 0, 2, 6, 7),
                                                    _mm256_setr_ps(0, 3, 4, 5, 1, 2, 6, 7),
                                                    _mm256_setr_ps(3, 4, 5, 0, 1, 2, 6, 7),
                                                    _mm256_setr_ps(0, 1, 2, 4, 5, 3, 6, 7),
                                                    _mm256_setr_ps(1, 2, 4, 5, 0, 3, 6, 7),
                                                    _mm256_setr_ps(0, 2, 4, 5, 1, 3, 6, 7),
                                                    _mm256_setr_ps(2, 4, 5, 0, 1, 3, 6, 7),
                                                    _mm256_setr_ps(0, 1, 4, 5, 2, 3, 6, 7),
                                                    _mm256_setr_ps(1, 4, 5, 0, 2, 3, 6, 7),
                                                    _mm256_setr_ps(0, 4, 5, 1, 2, 3, 6, 7),
                                                    _mm256_setr_ps(4, 5, 0, 1, 2, 3, 6, 7),
                                                    _mm256_setr_ps(0, 1, 2, 3, 5, 4, 6, 7),
                                                    _mm256_setr_ps(1, 2, 3, 5, 0, 4, 6, 7),
                                                    _mm256_setr_ps(0, 2, 3, 5, 1, 4, 6, 7),
                                                    _mm256_setr_ps(2, 3, 5, 0, 1, 4, 6, 7),
                                                    _mm256_setr_ps(0, 1, 3, 5, 2, 4, 6, 7),
                                                    _mm256_setr_ps(1, 3, 5, 0, 2, 4, 6, 7),
                                                    _mm256_setr_ps(0, 3, 5, 1, 2, 4, 6, 7),
                                                    _mm256_setr_ps(3, 5, 0, 1, 2, 4, 6, 7),
                                                    _mm256_setr_ps(0, 1, 2, 5, 3, 4, 6, 7),
                                                    _mm256_setr_ps(1, 2, 5, 0, 3, 4, 6, 7),
                                                    _mm256_setr_ps(0, 2, 5, 1, 3, 4, 6, 7),
                                                    _mm256_setr_ps(2, 5, 0, 1, 3, 4, 6, 7),
                                                    _mm256_setr_ps(0, 1, 5, 2, 3, 4, 6, 7),
                                                    _mm256_setr_ps(1, 5, 0, 2, 3, 4, 6, 7),
                                                    _mm256_setr_ps(0, 5, 1, 2, 3, 4, 6, 7),
                                                    _mm256_setr_ps(5, 0, 1, 2, 3, 4, 6, 7),
                                                    _mm256_setr_ps(0, 1, 2, 3, 4, 5, 6, 7),
                                                    _mm256_setr_ps(1, 2, 3, 4, 0, 5, 6, 7),
                                                    _mm256_setr_ps(0, 2, 3, 4, 1, 5, 6, 7),
                                                    _mm256_setr_ps(2, 3, 4, 0, 1, 5, 6, 7),
                                                    _mm256_setr_ps(0, 1, 3, 4, 2, 5, 6, 7),
                                                    _mm256_setr_ps(1, 3, 4, 0, 2, 5, 6, 7),
                                                    _mm256_setr_ps(0, 3, 4, 1, 2, 5, 6, 7),
                                                    _mm256_setr_ps(3, 4, 0, 1, 2, 5, 6, 7),
                                                    _mm256_setr_ps(0, 1, 2, 4, 3, 5, 6, 7),
                                                    _mm256_setr_ps(1, 2, 4, 0, 3, 5, 6, 7),
                                                    _mm256_setr_ps(0, 2, 4, 1, 3, 5, 6, 7),
                                                    _mm256_setr_ps(2, 4, 0, 1, 3, 5, 6, 7),
                                                    _mm256_setr_ps(0, 1, 4, 2, 3, 5, 6, 7),
                                                    _mm256_setr_ps(1, 4, 0, 2, 3, 5, 6, 7),
                                                    _mm256_setr_ps(0, 4, 1, 2, 3, 5, 6, 7),
                                                    _mm256_setr_ps(4, 0, 1, 2, 3, 5, 6, 7),
                                                    _mm256_setr_ps(0, 1, 2, 3, 4, 5, 6, 7),
                                                    _mm256_setr_ps(1, 2, 3, 0, 4, 5, 6, 7),
                                                    _mm256_setr_ps(0, 2, 3, 1, 4, 5, 6, 7),
                                                    _mm256_setr_ps(2, 3, 0, 1, 4, 5, 6, 7),
                                                    _mm256_setr_ps(0, 1, 3, 2, 4, 5, 6, 7),
                                                    _mm256_setr_ps(1, 3, 0, 2, 4, 5, 6, 7),
                                                    _mm256_setr_ps(0, 3, 1, 2, 4, 5, 6, 7),
                                                    _mm256_setr_ps(3, 0, 1, 2, 4, 5, 6, 7),
                                                    _mm256_setr_ps(0, 1, 2, 3, 4, 5, 6, 7),
                                                    _mm256_setr_ps(1, 2, 0, 3, 4, 5, 6, 7),
                                                    _mm256_setr_ps(0, 2, 1, 3, 4, 5, 6, 7),
                                                    _mm256_setr_ps(2, 0, 1, 3, 4, 5, 6, 7),
                                                    _mm256_setr_ps(0, 1, 2, 3, 4, 5, 6, 7),
                                                    _mm256_setr_ps(1, 0, 2, 3, 4, 5, 6, 7),
                                                    _mm256_setr_ps(0, 1, 2, 3, 4, 5, 6, 7),
                                                    _mm256_setr_ps(0, 1, 2, 3, 4, 5, 6, 7)};
 ...
 * update smallest and largest values in smallest_vec and biggest_vec respectively */
            inline int partition_vec(__m256 &curr_vec, const __m256 &pivot_vec,
                    __m256 &smallest_vec, __m256 &biggest_vec){
                /* which elements are larger than the pivot */
                __m256 compared = _mm256_cmpgt_epi32(curr_vec, pivot_vec); // TODO: find relevant intrinisc for float
                ...
                curr_vec = _mm256_permutevar8x32_ps(curr_vec, permutation_masks[mm]);
                /* return how many elements are greater than pivot */
                return amount_gt_pivot; }

            inline int calc_min(__m256 vec) { /* minimum of 8 int */
                auto perm_mask = _mm256_setr_ps(7, 6, 5, 4, 3, 2, 1, 0);
            ...
            inline int calc_max(__m256 vec){ /* maximum of 8 float */
                auto perm_mask = _mm256_setr_ps(7, 6, 5, 4, 3, 2, 1, 0);
                ...
                return _mm256_extract_epi32(vec, 0); // TODO: find equivalent of extract from index for float
            }

            inline int partition_vectorized_8(int *arr, int left, int right,
            int pivot, float &smallest, float &biggest) {
...
/* simulate wider vector registers to speedup sorting */
            inline int partition_vectorized_64(float *arr, int left, int right,
            int pivot, float &smallest, float &biggest) {
                ...
                for (int i = ((right - left) % 64) / 8; i > 0; --i) {
                    __m256 vec_L = LOAD_VECTOR(arr + left);
                    __m256 compared = _mm256_cmpgt_epi32(vec_L, pivot_vec);
                    sv = _mm256_min_ps(vec_L, sv); bv = _mm256_max_ps(vec_L, bv);
                    int mm = _mm256_movemask_ps(_mm256_castsi256_ps(compared));
                    int amount_gt_pivot = _mm_popcnt_u32((mm));
                    __m256 permuted = _mm256_permutevar8x32_ps(vec_L, permutation_masks[mm]);

                    /* this is a slower way to partition an array with vector instructions */
                    __m256 blend_mask = _mm256_cmpgt_epi32(permuted, pivot_vec);
                    __m256 vec_R = LOAD_VECTOR(arr + right - 8);
                    __m256 vec_L_new = _mm256_blendv_ps(permuted, vec_R, blend_mask);
                    __m256 vec_R_new = _mm256_blendv_ps(vec_R, permuted, blend_mask);
                ...
                while (right - left != 0) { /* partition 64 elements per iteration */
                    __m256 curr_vec, curr_vec2, curr_vec3, curr_vec4, curr_vec5, curr_vec6, curr_vec7, curr_vec8;
...
//
//            inline __m256 vnext(__m256 &s0, __m256 &s1) {
...
//            inline __m256 rnd_epu32(__m256 rnd_vec, __m256 bound) {
//                __m256 even = _mm256_srli_epi64(_mm256_mul_epu32(rnd_vec, bound), 32);
//                __m256 odd = _mm256_mul_epu32(_mm256_srli_epi64(rnd_vec, 32), bound);
//                return _mm256_blend_ps(odd, even, 0b01010101); }
//
...
//                auto bound = _mm256_set1_ps(right - left + 1);
//                auto left_vec = _mm256_set1_ps(left);
//
...
//                    result = _mm256_add_ps(result, left_vec); /* indices for arr */
//                    v[i] = _mm256_i32gather_ps(arr, result, sizeof(uint32_t)); }
//
...
/* recursion for quicksort */
            ...
            inline void qs_core(float *arr, int left, int right,
                                bool choose_avg = false, const int avg = 0) {
                if (right - left < 513) { /* use sorting networks for small arrays */
                    __m256 buffer[66]; /* buffer for sorting networks */
                    int* buff = reinterpret_cast<float *>(buffer);
...
/* call this function for sorting */
        inline void quicksort(float *arr, int n) { _internal::qs_core(arr, 0, n - 1); }
} /* namespace avx2 end */
